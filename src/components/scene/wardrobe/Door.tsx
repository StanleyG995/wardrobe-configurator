...
const getHingePositionsY = (height: number): number[] => {
  ...
  else if (height >= HEIGHT_BREAKPOINT_4 && height < HEIGHT_BREAKPOINT_5)
    count = 4;
...
const Door = ({
  ...
  const staticGeometry = useMemo(() => {
    ...
    const handlePos: [number, number, number] = [
      toMeters(
        isLeftHandle ? width - HANDLE_OFFSET_X : -width + HANDLE_OFFSET_X,
      ),
      0,
    ...
    const hingeArmPos: [number, number, number] = [
      toMeters(
        isLeftHinge
          ? -width / 2 + boardThickness + boardThickness / 3
          : width / 2 - boardThickness - boardThickness / 3,
      ),
      toMeters(height / 2 + topOffset),
      toMeters(depth),
    ...
    const hingeZPos = isLeftHandle
      ? toMeters(HINGE_Z_OFFSET)
      : toMeters(-HINGE_Z_OFFSET);
  ...
  useLayoutEffect(() => {
    if (hingeCupRef.current) {
      ...
      const destX = isLeftHinge
        ? staticGeometry.hingePos[0]
        : staticGeometry.hingePos[0];
  ...
  useFrame(() => {
    if (hingeCupRef.current) {
      ...
      const destX = isLeftHinge
        ? staticGeometry.hingePos[0] + targetHingePositionX
        : staticGeometry.hingePos[0] - targetHingePositionX;
      ...
      if (
        ...
        positionXDiff < POSITION_THRESHOLD
      ) {
        hingeCupRef.current.rotation.y = currentTargetRotation;
        hingeCupRef.current.position.x = destX;
        hingeCupRef.current.position.z = destZ;
  ...
  return (
    <group>
      <group position={staticGeometry.hingeArmPos}>
        {hingePositionsY.map((yPosition, index) => (
          <HingeArm
            ...
            position={[
              0,
              toMeters(yPosition - height / 2),
              -toMeters(depth / 2 + HINGE_ARM_OFFSET),
            ]}
      ...
      <group
        ...
        onClick={(e) => {
          handleViewportToggle("doorsOpen");
          e.stopPropagation();
        }}
      >
        <DoorHandle
        ...
        {hingePositionsY.map((yPosition, index) => (
          <group key={index}>
            <HingeCup
              position={[
                staticGeometry.hingeZPos,
                toMeters(yPosition - height / 2),
                toMeters(HINGE_CUP_Z),
              ]}
              scale={isLeftHinge ? [1, 1, 1] : [-1, 1, 1]}
