...
export default function Home() {
  ...
  return (
    <main
      className={cn(
        STYLES.main,
        isSidebarOpen ? "md:pr-[420px]" : "md:pr-0"
      )}
    >
      ...
      <button
        ...
        className={cn(
          STYLES.toggleBtn,
          isSidebarOpen ? "md:-translate-x-[420px]" : "md:translate-x-0"
        )}
      >
        {isSidebarOpen ? (
          <PiArrowRight className="translate-x-[1px] text-[12px]" />
        ) : (
          <PiArrowLeft className="-translate-x-[1px] text-[12px]" />
        )}
      </button>
    </main>
  );
...
const STYLES = {
  ...
  renderContainer: cn("relative h-[45vh] w-full bg-white touch-none md:h-full md:w-full"),
  
  
  sidebarContainer: cn(
    "absolute bottom-0 right-0 z-40 flex h-[55vh] w-full flex-col border-t border-gray-200 bg-white shadow-2xl transition-transform duration-300 ease-in-out overflow-hidden",
